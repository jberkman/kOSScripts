...

{
    function transfer {
        ...
        function eval {
            ...
            // (5.5)
            local f is 1 - r2 / p * (1 - cos(v)).
            // (5.6)
            local g is r1 * r2 * sin(v) / sqrt(body:mu * p).
            ...
            if a > 0 {
                // (5.13)
                local dE is arccos(1 - r1 * (1 - f) / a).
                // (5.16)
                set t to g + sqrt((a ^ 3) / body:mu) * (dE - sin(dE)).
            } else {
                // (5.15)
                local dF is arccosh(1 - r1 / a * (1 - f)).
                // (5.17)
                set t to g + sqrt((-a ^ 3) / body:mu) * (sinh(dF) - dF).
            }
        ...
        until abs(duration - pat["t"]) < duration / 1000 {
        ...
        self:add("captureVector", captureVector@:bind(self)).
        self:add("departureVector", departureVector@:bind(self)).
        self:add("orbit", orbit@:bind(self)).
